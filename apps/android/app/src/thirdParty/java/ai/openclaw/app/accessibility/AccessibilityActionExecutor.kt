...
class AccessibilityActionExecutor internal constructor(
  ...
  private val captureSnapshot: (OpenClawAccessibilityService, MobileUiTarget) -> AccessibilitySnapshotCapture =
    AccessibilitySnapshotter()::capture,
  private val targetPackageProvider: (OpenClawAccessibilityService, MobileUiTarget) -> String? =
    OpenClawAccessibilityService::packageNameForTarget,
  private val uiEpochProvider: (MobileUiTarget) -> Long = OpenClawAccessibilityService::uiEpoch,
) : AutoCloseable {
  ...
  fun observe(target: MobileUiTarget): MobileUiSnapshot {
    ...
    val capturedUiEpoch = uiEpochProvider(target)
    val capturedConnectionGeneration = capturedConnection.generation
    val capture = captureSnapshot(service, target)
    synchronized(generationLock) {
  ...
  suspend fun act(
    ...
    if (action is MobileUiAction.GlobalAction) {
      if (target is MobileUiTarget.VScreenDisplay) {
        return ActionResult(
          ActionOutcomeCode.ActionNotSupported,
          "Display-targeted global actions are not available yet",
        )
      }
    ...
    synchronized(generationLock) {
      ...
      if (generation.target != target) {
        return ActionResult(ActionOutcomeCode.TargetStale, "Android target changed; observe again before acting")
      }
      ...
      when (action) {
        ...
        -> coordinateGesturePreflight(service, target)?.let { return it }
        ...
        -> nodeActionPackagePreflight(service, target)?.let { return it }
    ...
    return when (action) {
      ...
      is MobileUiAction.Tap -> {
        if (target is MobileUiTarget.VScreenDisplay && Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
          return ActionResult(
            ActionOutcomeCode.ActionNotSupported,
            "Safe VScreen gestures require Android 13 or newer",
          )
        }
        val gesture =
          runCatching { tapGesture(action.x, action.y, target.displayId) }
      ...
      is MobileUiAction.Swipe -> {
        if (action.durationMs <= 0) {
          ...
          val gesture =
            runCatching { swipeGesture(action, target.displayId) }
  ...
  private fun coordinateGesturePreflight(
    service: OpenClawAccessibilityService,
    target: MobileUiTarget,
  ): ActionResult? {
    val expectedPackage = generation.packageName
    val currentPackage = targetPackageProvider(service, target)
    ...
    if (uiEpochProvider(target) > generation.uiEpoch) {
      return ActionResult(
  ...
  private fun nodeActionPackagePreflight(
    service: OpenClawAccessibilityService,
    target: MobileUiTarget,
  ): ActionResult? {
    val expectedPackage = generation.packageName
...
internal class SnapshotGenerationStore<T>(
  ...
  var connectionGeneration: Long = 0
    private set
  var target: MobileUiTarget? = null
    private set
...
private fun tapGesture(
  ...
  return gestureBuilder(displayId)
    .addStroke(GestureDescription.StrokeDescription(path, 0, 1))
    .build()
}

private fun swipeGesture(
  action: MobileUiAction.Swipe,
  displayId: Int,
): GestureDescription {
  val path =
    Path().apply {
