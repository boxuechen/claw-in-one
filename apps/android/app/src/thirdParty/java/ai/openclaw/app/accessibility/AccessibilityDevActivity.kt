...
class AccessibilityDevActivity : AppCompatActivity() {
  ...
  private suspend fun captureSnapshotOffMain(): MobileUiSnapshot =
    withContext(Dispatchers.Default) {
      ...
      val packageName = OpenClawAccessibilityService.instance?.foregroundPackageName() ?: error("No active Android app")
      executor.observe(MobileUiTarget.MainDisplay(packageName))
    }
}
...
private fun AccessibilityDevScreen(
  ...
  fun act(action: MobileUiAction) {
    ...
    coroutineScope.launch {
      lastResult = executor.act(activeSnapshot.target, activeSnapshot.id, action)
    }
  }
    ...
    coroutineScope.launch {
      lastResult =
        executor.act(
          ...
          activeSnapshot.id,
          MobileUiAction.GlobalAction(name),
        )
    }
  }
