...
class ChatControllerSessionSearchTest {
  ...
  fun filterSessionEntriesMatchesDisplayNameLabelAndKey() {
    val sessions =
      listOf(
        ...
        ChatSessionEntry(key = "agent:main:topic-b", updatedAtMs = 1, displayName = "Groceries", label = "Shopping"),
    ...
    assertEquals(listOf("agent:main:topic-b"), filterSessionEntries(sessions, "SHOPPING").map { it.key })
  ...
  fun fetchSessionListSendsSearchAndArchivedParams() =
    runTest {
      ...
      val archivedRows = controller.sessionCatalogFeature.search(null, true)
      assertEquals(listOf("agent:main:old"), archivedRows.map { it.key })
      assertTrue(archivedRows.single().archived == true)

      controller.sessionCatalogFeature.search("  trip  ", false)
  ...
  fun fetchSessionListPreservesGatewayClassificationFacts() =
    runTest {
      ...
      val row = controller.sessionCatalogFeature.search(null, false).single()
  ...
  fun fetchSessionListFallsBackToLocalFilterWhenOffline() =
    runTest {
      ...
      val filtered = controller.sessionCatalogFeature.search("trip", false)
      ...
      assertTrue(controller.sessionCatalogFeature.search(null, true).isEmpty())
    }
  ...
  fun fetchSessionListDoesNotGuessMainWhileDefaultOwnerIsUnknown() =
    runTest {
      ...
      assertTrue(controller.sessionCatalogFeature.search("trip", false).isEmpty())
      assertTrue(controller.sessionCatalogFeature.search(null, true).isEmpty())
  ...
  fun fetchSessionListDropsAResponseAfterOwnerOrGatewayChanges() =
    runTest {
      ...
      val pending = async { controller.sessionCatalogFeature.search("trip", false) }
      runCurrent()
