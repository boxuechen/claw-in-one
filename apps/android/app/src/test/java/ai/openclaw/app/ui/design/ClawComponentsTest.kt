...
class ClawComponentsTest {
  @Test
  fun sharedSizingKeepsInteractiveControlsAccessible() {
    val sizes = ClawSizes()

    assertTrue(sizes.minimumTouchTarget >= 48.dp)
    assertTrue(sizes.floatingControl >= sizes.minimumTouchTarget)
    assertTrue(sizes.prominentIcon > sizes.standardIcon)
    assertTrue(sizes.standardIcon > sizes.compactIcon)
  }
