...
class ChatTimelineTest {
      ...
      buildChatTimeline(
        ...
        supplement =
          ChatTimelineSupplement(
            currentWork = ChatCurrentWorkUiState(progress = null, tools = emptyList()),
          ),
      )

    assertEquals(listOf("current-work", "message:user-1"), timeline.items.map(::chatTimelineItemKey))
  ...
  fun activeRunAnchorsNewestUserPromptWhileAssistantStreams() {
    ...
    val timeline =
      buildChatTimeline(
        ...
        answerDraft = ChatAssistantAnswerDraft("run-1", "streaming"),
        supplement =
          ChatTimelineSupplement(
    ...
    assertEquals(
      listOf("answer-draft:run-1", "current-work", "message:user-1", "message:assistant-1"),
  ...
  fun finishedRunKeepsLatestUserPromptAsReaderAnchor() {
    ...
    assertEquals(0, timeline.latestContentIndex)
    assertEquals("user-1", timeline.latestUserMessageId)
    assertTrue(timeline.items.first() is ChatTimelineItem.AssistantAnswer)
  }

  @Test
  fun nonRenderableActivityMessagesNeverBecomeSpacedTimelineRows() {
    val user = textMessage(id = "user-1", role = "user", text = "Present the app")
    val hiddenActivity =
      listOf(
        ChatMessage(id = "empty", role = "assistant", content = emptyList(), timestampMs = 2L),
        ChatMessage(
          id = "blank",
          role = "assistant",
        ...
        ChatMessage(
          id = "tool",
          role = "assistant",
          content = listOf(ChatMessageContent(type = "toolCall", text = "exec")),
          timestampMs = 4L,
        ),
      )
    val answer = textMessage(id = "assistant-1", role = "assistant", text = "The app is ready")

    val timeline =
      buildChatTimeline(
        messages = listOf(user) + hiddenActivity + answer,
        pendingRunCount = 0,
        pendingToolCalls = emptyList(),
        answerDraft = null,
      )
  ...
  fun transcriptMarkersBecomeStableDividersAndUnknownKindsStayHidden() {
    ...
    assertEquals(
      "system",
      timeline.items
        .filterIsInstance<ChatTimelineItem.TranscriptMessage>()[1]
        .message.role,
    )
  ...
  @Test
  fun activeLiveToolDiffChangesTimelineContentVersion() {
    ...
    val initial =
      buildChatTimeline(
        ...
        supplement =
          ChatTimelineSupplement(
            currentWork = ChatCurrentWorkUiState(progress = null, tools = listOf(pending)),
          ),
      )
    val updatedTool = pending.copy(liveDiff = ChatDiffStat(added = 4, removed = 1))
    val updated =
      buildChatTimeline(
        messages = emptyList(),
        pendingRunCount = 1,
        pendingToolCalls = listOf(updatedTool),
        answerDraft = null,
        supplement =
          ChatTimelineSupplement(
            currentWork = ChatCurrentWorkUiState(progress = null, tools = listOf(updatedTool)),
          ),
      )
