...
class ChatComposerDraftTest {
  @Test
  fun projectStarterStagesAnEditableDraftWithoutBeginningSend() {
    ...
    val starter =
      ProjectStarter(
        id = "android-kotlin",
        capability = DevelopmentCapability.AndroidKotlin,
        icon = ProjectStarterIcon.Kotlin,
        title = "Kotlin App",
        skillReference = "android-development",
        prompt = "Build a small Kotlin app",
      )

    assertTrue(state.stageProjectStarter(owner, starter))

    assertEquals("Build a small Kotlin app", state.textDrafts[owner])
    assertEquals(listOf("android-development"), state.textDrafts.skillReferences(owner))
    assertTrue(state.sendStates.value.isEmpty())
    assertFalse(state.stageProjectStarter(owner, starter))
  }

  @Test
  fun skillMentionsAreOwnerScopedDeduplicatedAndRestoredWithTheDraft() {
    ...
    store[first] = "Build an app"
    store.selectSkillReference(first, "android-development")
    store.selectSkillReference(first, "android-development")
    store.selectSkillReference(second, "review")

    val restored = ChatComposerTextDraftStore(initial = chatComposerTextDraftsFromSnapshot(saved))

    assertEquals(listOf("android-development"), restored.skillReferences(first))
    assertEquals(listOf("review"), restored.skillReferences(second))
    assertEquals("Build an app", restored[first])
  }

  @Test
  fun skillMentionsUseTheNormalSendCheckpointAndRestoreAfterRejection() {
    val owner = ChatComposerOwner(gatewayStableId = "gateway-a", agentId = "main", sessionKey = "agent:main:first")
    val state = ChatComposerStateStore()
    state.textDrafts[owner] = "Build it"
    state.textDrafts.selectSkillReference(owner, "android-development")

    val request = requireNotNull(state.beginSend(owner).request)
    assertEquals("\$android-development\n\nBuild it", request.message)
    assertEquals(emptyList<String>(), state.textDrafts.skillReferences(owner))

    state.completeSend(request, accepted = false)
    assertEquals("Build it", state.textDrafts[owner])
    assertEquals(listOf("android-development"), state.textDrafts.skillReferences(owner))
  }
  ...
  fun imagePickerCheckpointCarriesTheCredentialGenerationThroughRecreation() {
    ...
    val checkpoint = ChatComposerAttachmentCheckpoint()
    checkpoint.begin(owner, attachmentAuthorizationId = "attachment-auth")
    val saverScope = SaverScope { true }
    val saved =
      with(ChatComposerAttachmentCheckpoint.Saver) {
        saverScope.save(checkpoint)
      }
    val restored = requireNotNull(ChatComposerAttachmentCheckpoint.Saver.restore(requireNotNull(saved)))

    assertEquals(ChatComposerAttachmentLease(owner, "attachment-auth"), restored.consume())
  }
  ...
  @Test
  fun productDraftMigrationKeepsRejectedFirstTurnWithTheCreatedSession() {
    val draftOwner =
      ChatComposerOwner(
        gatewayStableId = "gateway-a",
        agentId = "main",
        sessionKey = "product-new-chat:draft-a",
      )
    val runtimeOwner = draftOwner.copy(sessionKey = "agent:main:created")
    val attachment = pendingAttachment("first-turn")
    val state = ChatComposerStateStore()
    state.textDrafts[draftOwner] = "first turn"
    state.addAttachments(draftOwner, listOf(attachment))
    val request = requireNotNull(state.beginSend(draftOwner).request)

    state.migrateOwner(from = draftOwner, to = runtimeOwner)

    assertEquals("", state.textDrafts[draftOwner])
    assertEquals(listOf(attachment), state.attachments.value[runtimeOwner])
    assertEquals(setOf(request.commandId), state.sendStates.value[runtimeOwner]?.activeOperationIds)

    state.completeSend(request, accepted = false)

    assertEquals("first turn", state.textDrafts[runtimeOwner])
    assertEquals(listOf(attachment), state.attachments.value[runtimeOwner])
    assertEquals(setOf(request.commandId), state.sendStates.value[runtimeOwner]?.pendingAdmissionIds)
    state.acknowledgeSendAdmission(runtimeOwner, request.commandId)
    assertNull(state.sendStates.value[runtimeOwner])
  }
