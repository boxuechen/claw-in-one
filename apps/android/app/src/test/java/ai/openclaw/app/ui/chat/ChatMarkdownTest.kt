...
class ChatMarkdownTest {
  ...
  @Test
  fun dollarAndBracketNotationUseOrdinaryMarkdownParsing() {
    val dollar =
      (parseChatMarkdownBlocks("$$ x + y $$").single() as ChatMarkdownRenderBlock.CommonMark).node as Paragraph
    val bracket =
      (parseChatMarkdownBlocks("\\[x + y\\]").single() as ChatMarkdownRenderBlock.CommonMark).node as Paragraph
    val code =
      (parseChatMarkdownBlocks("`$$ x + y $$`").single() as ChatMarkdownRenderBlock.CommonMark).node as Paragraph

    assertEquals("$$ x + y $$", (dollar.firstChild as org.commonmark.node.Text).literal)
    assertEquals("[x + y]", (bracket.firstChild as org.commonmark.node.Text).literal)
    assertEquals("$$ x + y $$", (code.firstChild as Code).literal)
  }
