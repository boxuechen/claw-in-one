...
class ChatCodeBlockLayoutTest {
  ...
  fun shortCodeKeepsItsCompleteHighlightedMonospaceLayoutWithoutNavigation() {
    ...
    listOf("Start of code", "End of code").forEach { label ->
      composeRule.onNodeWithText(label).assertDoesNotExist()
    }
    composeRule.onNode(hasContentDescription("Copy code") and hasClickAction()).assertIsDisplayed()
  }
  ...
  private fun assertCopiedSource(expected: String) {
    ...
    composeRule.onNode(hasContentDescription("Copy code") and hasClickAction()).assertIsDisplayed().performClick()
