...
internal fun drainWithMainLooper(block: suspend () -> Unit) {
  ...
  try {
    runBlocking {
      ...
      // poll() unconditionally waits, even when a callback was queued before entering it.
      // Drain first and bound the wait: a completion between idle() and poll() must not
      // strand Main after the final worker has already posted its only wakeup.
      while (!completed) {
        shadowLooper.idle()
        if (!completed) shadowLooper.poll(10)
      }
      cleanup.await()
    }
  } finally {
