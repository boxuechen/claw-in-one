...
@Database(
  ...
  // v4 intentionally drops v3 transcript rows: v3 could not distinguish Gateway commentary
  // projections from durable answers. This database is disposable; client-owned state is separate.
  version = 4,
  exportSchema = true,
)
  ...
  companion object {
    internal val MIGRATION_1_2 =
      object : Migration(1, 2) {
        override fun migrate(db: SupportSQLiteDatabase) = migrateOutboxAttachmentsWithoutDuration(db)
      }
...
private fun migrateOutboxAttachmentsWithoutDuration(db: SupportSQLiteDatabase) {
  db.execSQL(
    "CREATE TABLE IF NOT EXISTS `outbox_attachments_new` (`id` TEXT NOT NULL, `commandId` TEXT NOT NULL, " +
      "`position` INTEGER NOT NULL, `type` TEXT NOT NULL, `mimeType` TEXT NOT NULL, `fileName` TEXT NOT NULL, " +
      "`byteLength` INTEGER NOT NULL, PRIMARY KEY(`id`))",
  )
  db.execSQL(
    "INSERT INTO `outbox_attachments_new` (`id`, `commandId`, `position`, `type`, `mimeType`, `fileName`, `byteLength`) " +
      "SELECT `id`, `commandId`, `position`, `type`, `mimeType`, `fileName`, `byteLength` FROM `outbox_attachments`",
  )
  db.execSQL("DROP TABLE `outbox_attachments`")
  db.execSQL("ALTER TABLE `outbox_attachments_new` RENAME TO `outbox_attachments`")
  db.execSQL("CREATE INDEX IF NOT EXISTS `index_outbox_attachments_commandId` ON `outbox_attachments` (`commandId`)")
}
