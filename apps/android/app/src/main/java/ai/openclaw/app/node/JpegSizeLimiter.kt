...
internal object JpegSizeLimiter {
  ...
  /** Applies imported-image orientation before resize, display, or metadata-stripping JPEG encoding. */
  ...
  fun compressToLimit(
    ...
    if (failed.bytes.size > maxBytes) {
      throw IllegalStateException("IMAGE_TOO_LARGE: ${failed.bytes.size} bytes > $maxBytes bytes")
    }
