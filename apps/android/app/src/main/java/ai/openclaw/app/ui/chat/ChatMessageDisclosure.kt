...
internal fun ChatMessageDisclosure(
  ...
  // leaving ordinary bubble expansion and menus at their existing owners.
  val scope = key(source, owner, selectionGeneration, catalogRevision) { rememberCoroutineScope() }
  ...
  fun open(
    ...
    val admitted = source?.prepare?.invoke(owner, selectionGeneration, catalogRevision, message) ?: return
    onManualNavigation()
  ...
  content(
    { message ->
      if (expanded && active?.message?.matchesFullRead(message) == true && result is ChatFullMessageState.Loaded) {
        // History can append non-text artifacts from later transcript rows.
