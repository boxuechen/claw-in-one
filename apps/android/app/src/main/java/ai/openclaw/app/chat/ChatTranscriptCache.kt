...
internal data class CachedMessageEntity(
  ...
  val rowOrder: Int,
  val role: String,
  // JSON payload of text, managed-image references, and inert attachment metadata.
 ...
 * before their suspend point, so pairing replacement cannot re-scope an old response.
 */
class RoomChatTranscriptCache internal constructor(
  ...
  override suspend fun loadTranscript(
    ...
    return database.dao().messages(gateway, agent, key).mapNotNull { row ->
      ...
      val payload = decodeCachedMessage(row.textPartsJson) ?: return@mapNotNull null
      ChatMessage(
  ...
  override suspend fun saveTranscript(
    ...
    // Persist small image references and inert attachment metadata, never attachment bytes.
    val rows =
      messages
        .mapNotNull { message ->
          ...
          val content =
            message.content.mapNotNull { part ->
              when {
                ...
                part.type == "file" || part.type == "unsupported" ->
                  CachedMessageContent(
                    type = part.type,
                    mimeType = part.mimeType,
                    fileName = part.fileName,
  ...
  private fun decodeCachedMessage(encoded: String): CachedMessagePayload? = runCatching { json.decodeFromString(cachedPayloadSerializer, encoded) }.getOrNull()
}
